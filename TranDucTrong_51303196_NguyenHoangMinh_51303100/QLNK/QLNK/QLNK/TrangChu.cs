    ...
    public partial class TrangChu : MaterialForm
    {
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            if (this.tabPage1.Focus())
            {
                if (listView1.SelectedIndices.Count > 0)
                {
                    ...
                    f3.ShowDialog();
                    sc.displayNhanKhau(listView1, 0);
                    this.Show();
                }
                else
                {
        ...
        private void tabpageSelect(int tabpageIndex)
        {
                ...
                tabpageIndex == 3 || tabpageIndex == 4 || tabpageIndex == 5)
            {
                hideTabpageTraCuu();
                hideTabpageTaiKhoan();
                if (tabpageIndex == 0) { sc.displayNhanKhau(listView1, 0); }
                else if (tabpageIndex == 1) { sc.displayHoKhau(listView2, 0); }
        ...
        private void setAuthorities(string phanQuyen)
        {
            switch (phanQuyen)
            {
                ...
                case "2": this.materialTabControl1.SelectedTab = tabPage1; sc.displayNhanKhau(listView1, 0); flagNhanKhau = true; materialTabControl1.TabPages.Remove(tabPage8); break;
                case "3": this.materialTabControl1.SelectedTab = tabPage2; sc.displayHoKhau(listView2, 0); flagHoKhau = true; materialTabControl1.TabPages.Remove(tabPage8); break;
